% ============================================================
%  GUÍA 1: EL ARTE DE DERIVAR — REGLAS Y DEFINICIÓN  (variante Δx)
...
\mwguia{Clase 1 — Reglas de Derivación y Definición ($\Delta x$)}{Ing. Gabriel Astudillo $\cdot$ Minicurso de Derivadas}
...
\begin{cuadroinfo}
  \small
  \textbf{Presentación:} En la primera clase del minicurso dominaremos la mecánica de la derivación. Empezamos con la \emph{definición} de derivada como límite (escrita en su forma equivalente con el incremento $\Delta x$ en lugar de $h$) y luego las \emph{reglas} que nos permiten derivar con velocidad: potencia, producto, cociente y la regla de la cadena, con énfasis en funciones compuestas anidadas. Aquí se construye la herramienta que usaremos en las próximas clases: la interpretación geométrica, la derivación implícita, las paramétricas y el estudio completo de curvas.
  \\[6pt]
...
\begin{cuadrosolucion}
  ...
  \[
    ...
    = \frac{\dfrac{2(x+\Delta x)}{1-(x+\Delta x)} - \dfrac{2x}{1-x}}{\Delta x}
  \]
  \textbf{Paso 2:} Simplificamos el numerador (común denominador $(1-x)(1-x-\Delta x)$).
  \[
    \frac{f(x+\Delta x) - f(x)}{\Delta x}
    = \frac{2(x+\Delta x)(1-x) - 2x(1-x-\Delta x)}{\Delta x\,(1-x)(1-x-\Delta x)}
  \]
  \textbf{Paso 3:} Desarrollamos el numerador.
  \[
    2x - 2x^2 + 2\Delta x - 2\Delta x\,x - 2x + 2x^2 + 2\Delta x\,x = 2\Delta x
  \]
  \textbf{Paso 4:} Simplificamos el factor $\Delta x$ y tomamos límite.
  \[
    \lim_{\Delta x\to0} \frac{2\Delta x}{\Delta x\,(1-x)(1-x-\Delta x)} = \lim_{\Delta x\to0} \frac{2}{(1-x)(1-x-\Delta x)} = \frac{2}{(1-x)^2}
  \]
